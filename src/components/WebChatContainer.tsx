...
import { chatCompletion } from "@/utils/webChat"
import { chatCompletion as chatCompletionNormal } from "@/utils/chat"
...
const topTip = `您好！欢迎使用人工智能助手，请根据网页中内容提出您的问题。`
...
export default function ChatContainer(props: ChatProps) {
  ...
  function updateSessionInfo(list: MessageProps[], isNessary: boolean = false) {
    if (session.session_name && session.session_name.startsWith('未命名会话')) {
      ...
      chatCompletionNormal([titlePrompt], (message, isComplete) => {
        if (isComplete) {
          let name = message
  ...
  function handleSendMessage() {
    ...
    chatCompletion(sendList, session.params, session.id, (message, isComplete) => {
      setWaiting(false)
